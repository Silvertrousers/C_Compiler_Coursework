...
%type<string> T_AUTO T_BREAK T_CASE T_CHAR T_CONST T_CONTINUE T_DEFAULT T_DO T_DOUBLE
%type<string> T_ELSE T_ENUM T_EXTERN T_FLOAT T_FOR T_GOTO T_IF T_INT T_LONG T_REGISTER T_RETURN
%type<string> T_SHORT T_SIGNED T_SIZEOF T_STATIC T_STRUCT T_SWITCH
%type<string> T_TYPEDEF T_UNION T_UNSIGNED T_VOID T_VOLATILE T_WHILE
%type<string> T_TIMES T_DIVIDE T_PLUS T_MINUS T_EXPONENT T_DECREMENT T_INCREMENT
%type<string> T_LESS_THAN T_GREATER_THAN T_EQUALS T_AND T_OR T_BITWISE_NOT T_LOGICAL_NOT
%type<string> T_AND_AND T_OR_OR
%type<string> T_MODULO_EQUALS T_LEFT_SHIFT_EQUALS T_RIGHT_SHIFT_EQUALS T_NOT_EQUALS_EQUALS T_EQUALS_EQUALS
%type<string> T_PLUS_EQUALS T_MINUS_EQUALS T_TIMES_EQUALS T_DIVIDE_EQUALS
%type<string> T_LESS_THAN_OR_EQUAL_TO T_GREATER_THAN_OR_EQUAL_TO
%type<string> T_AND_EQUALS T_OR_EQUALS T_XOR_EQUALS
%type<string> T_ARROW T_RIGHT_SHIFT T_LEFT_SHIFT
%type<string> T_MODULO T_COMMA T_DOT T_COLON T_SEMICOLON T_QUESTION
%type<string> T_LBRACKET T_RBRACKET T_LSQ_BRACKET T_RSQ_BRACKET T_RCURLY_BRACKET T_LCURLY_BRACKET
%type<string> T_EMPTY_BRACKETS T_ECURLY_BRACKETS T_ESQ_BRACKETS
%type<string> T_DEC_INT T_OCTAL_INT T_IDENTIFIER T_STRING T_ENUM_CONSTANT
%type<string> T_DEREFERENCE T_CUSTOM_TYPE
...
TRANSLATION_UNIT : EXTERNAL_DECLARATION {$$ = new ast_node("TRANSLATION_UNIT","",$1)}
                 | TRANSLATION_UNIT EXTERNAL_DECLARATION {$$ = new ast_node("TRANSLATION_UNIT","", std::vector<nodePtr>{$1, $2})}
...
PRIMARY_EXPRESSION : T_IDENTIFIER {$$ = new ast_node("PRIMARY_EXPRESSION",$1)}
                   | CONSTANT {$$ = new ast_node("PRIMARY_EXPRESSION","",std::vector<nodePtr>{$1})}
                   | T_STRING
                   | T_LBRACKET EXPR T_RBRACKET {$$ = new ast_node("PRIMARY_EXPRESSION","",std::vector<nodePtr>{$2})}
