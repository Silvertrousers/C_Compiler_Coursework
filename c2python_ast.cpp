#ifndef c2python_ast_cpp
#define c2python_ast_cpp
#include <iostream>
#include <string>
...
void c2python_ast_node::print_python(int tab_count, c2python_symbol_table& table){
  ...
  for(int i=0;i<branches.size();i++){
    if(branches[i] == NULL){
      c2python_ast_node * temp = new c2python_ast_node("NULL","");
      branches[i] = temp;
    }
  }
  ...
  if(node_type == "FUNCTION_DECLARATION"){
    ...
    branches[1]->print_python(tab_count, table);//fn name and args
    std::cout<<":"<<std::endl;
